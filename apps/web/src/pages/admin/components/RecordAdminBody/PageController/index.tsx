...
import { setRecordCurPage as setCurPage } from '../../../../../app/store/pages/admin';
...
const getPageUnitFlags = (curPage: number, totalPageCount: number) => {
  if ([0, 1, 2, totalPageCount - 1, totalPageCount - 2, totalPageCount - 3].includes(curPage)) {
...
export const PageController = () => {
  const { curPage, totalPageCount } = useAppSelector((state) => state.adminPage.bodies.recordBody);
  ...
  return (
    ...
    >
      ...
      <IconButton aria-label="forward" onClick={forward} disabled={!(curPage < totalPageCount - 1)}>
